"""Claude Code 任务期间的背单词弹窗。

由 UserPromptSubmit 钩子（hook_start.py）启动，Stop 钩子（hook_stop.py）通知退出。
...
学习状态（进度、打标、测验成绩、历史）持久化在同目录 state.json。
...
# 老版本把 state.json 放在程序目录，存在就继续用，原地兼容
_LOCAL_STATE = os.path.join(APP_DIR, "state.json")
STATE_FILE = _LOCAL_STATE if os.path.exists(_LOCAL_STATE) else os.path.join(
    os.path.expanduser("~"), ".vocab_popup", "state.json")
