...
```
Claude Code 会话 A ──UserPromptSubmit──> hook_start.py ──"start A"──┐
Claude Code 会话 B ──UserPromptSubmit──> hook_start.py ──"start B"──┤
                                                                    v
                                                    vocab_popup.py（tkinter 单实例）
                                                    监听 127.0.0.1:<PORT> 控制通道
                                                                    ^
Claude Code 会话 A ──Stop──> hook_stop.py ──"stop A"────────────────┤
Claude Code 会话 B ──Stop──> hook_stop.py ──"stop B"（全空才关窗）──┘
```
...
- `hook_start.py`：读 session_id → 发 `start`；无应答则启动 `vocab_popup.py`
  （pythonw 优先，回退 python + CREATE_NO_WINDOW），最长重试 6 秒。
- `hook_stop.py`：读 session_id → 发 `stop`；无弹窗运行则静默退出。
...
位置：`~/.vocab_popup/state.json`（若程序目录有旧的 `state.json` 则原地沿用）。
...
### 测验抽题（`pick_quiz_word` / `quiz_weight`）

题库 = 打过标的词。权重：

```
w = 1 + (unknown: +3 | fuzzy: +1.5) + max(0, 答错次数 - 0.75×答对次数)，封顶 8
```

答对会抵消历史答错，防止"早期错过就永远霸榜"。最近考过的 3 个词不重复抽
（题库全被排除时回退全池）。答对自动进下一题（选择 1.4s / 默写 1s）；
答错/不会**不自动跳题**，手动继续。
