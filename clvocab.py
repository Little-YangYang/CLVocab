...
词表范围：顶栏「全部/生词/已背」循环切换，三种模式共用并持久化。
生词=未打标（背词时纯刷新词、打完标自动退出范围），已背=打过标（纯复习/只考已背）。
...
# 词表范围：三种模式共用，顶栏循环切换，持久化。生词=未打标，已背=打过标。
SCOPES = (("all", "全部"), ("new", "生词"), ("studied", "已背"))
SCOPE_EMPTY_HINTS = {
    "new": "生词表是空的：这本词书的词都打过标了",
    "studied": "已背词表是空的：先在「背词」模式给单词打标（认识/模糊/不认识）",
    "all": "词书是空的",
}
...
def filter_scope(words, stats, scope):
    """按词表范围过滤：all=全部；new=生词（未打标）；studied=已背（打过标）。"""
    if scope == "new":
        return [w for w in words if not stats.get(w["word"], {}).get("mark")]
    if scope == "studied":
        return [w for w in words if stats.get(w["word"], {}).get("mark")]
    return list(words)


def pick_quiz_word(words, stats, exclude=(), now=None, marked_only=True):
    """按权重抽一个测验词；无词可抽时返回 None。

    marked_only=True 时只考打过标（=已背）的词；调用方传入已按范围
    过滤的词表时用 False。exclude 里的词（最近刚考过的）不抽，除非全被排除。
    """
    now = time.time() if now is None else now
    pool = [w for w in words if stats.get(w["word"], {}).get("mark")] if marked_only else list(words)
    if not pool:
        return None
...
class App:
    def __init__(self, root, dicts, store):
        ...
        self.scope = store.get_setting("scope", "all")
    ...
    def _build_topbar(self):
        ...
        self.scope_btn = tk.Button(bar, relief="flat", bg=T["btn"], fg=T["fg"],
                                   activebackground=T["btn_active"], activeforeground=T["fg"],
                                   font=("Microsoft YaHei UI", 9), padx=6,
                                   command=self.cycle_scope)
        self.scope_btn.pack(side="left", padx=(6, 0))
        self._render_scope_btn()
    ...
    # ---- 词表范围 ----
    def scoped_words(self):
        return filter_scope(self.words, self.store.dict_words(self.dict_name), self.scope)

    def cycle_scope(self):
        keys = [k for k, _ in SCOPES]
        self.scope = keys[(keys.index(self.scope) + 1) % len(keys)]
        self.store.set_setting("scope", self.scope)
        self._render_scope_btn()
        self.current = None
        self.back_stack = []
        self.set_mode(self.mode)

    def _render_scope_btn(self):
        self.scope_btn.config(text=dict(SCOPES)[self.scope])
    ...
    def _update_status(self):
        ...
        scope_label = dict(SCOPES)[self.scope]
        self.status.config(
            text=f"{self.dict_name}·{scope_label} {len(self.scoped_words())} 词 · "
                 f"已背 {studied} · 今日背 {marked_today} 测 {quiz}")
    ...
    def _build_study(self):
        ...
        scoped = self.scoped_words()
        if not scoped:
            self.current = None
            tk.Label(c, text=SCOPE_EMPTY_HINTS[self.scope], bg=T["card"], fg=T["dim"],
                     font=("Microsoft YaHei UI", 11), wraplength=480).pack(expand=True)
            return
        ...
        self.word_label = tk.Label(c, bg=T["card"], fg=T["fg"], font=("Segoe UI", 25, "bold"), wraplength=480)
        ...
        names = {w["word"] for w in scoped}
        if self.current is not None and self.current["word"] in names:
            self.show_entry(self.current)  # 换主题/切模式回来时保留当前卡片
        else:
            entry, _ = self._linear_entry()
            if entry["word"] not in names:
                entry = self._advance_linear()
    ...
    def _advance_linear(self):
        """推进线性进度到下一个词；范围外的词跳过（生词/已背模式）。"""
        names = None
        if self.scope != "all":
            names = {w["word"] for w in self.scoped_words()}
            if not names:
                return None
        ...
        for _ in range(len(self.words)):
            ...
            entry = self.words[prog["order"][prog["pos"]]]
            if names is None or entry["word"] in names:
                return entry
        return entry
    ...
    def _draw_next(self):
        ...
        全部在当前词表范围内进行：生词模式没有复背池（纯刷新词），
        已背模式的"新词"就是已背词表的线性轮转。范围为空返回 (None, False)。
        """
        scoped = self.scoped_words()
        if not scoped:
            return None, False
        ...
        pools = build_review_pools(scoped, self.store.dict_words(self.dict_name), recent)
    ...
    def _new_choice(self):
        ...
        entry = pick_quiz_word(self.scoped_words(), stats, exclude=self.quiz_recent,
                               marked_only=False)
        if entry is None:
            self.choice_prompt.config(text="题库是空的")
            self.choice_phonetic.config(text=SCOPE_EMPTY_HINTS[self.scope])
            return
        T = self.T
        ...
        for meaning, is_correct in make_choices(self.words, entry):
            b = tk.Button(self.choice_frame, text=trim(meaning), anchor="w", justify="left",
                          ...
                          relief="flat", font=("Microsoft YaHei UI", 10), wraplength=450, padx=8)
    ...
    def _new_spell(self):
        ...
        entry = pick_quiz_word(self.scoped_words(), stats, exclude=self.quiz_recent,
                               marked_only=False)
        if entry is None:
            self.spell_meaning.config(text=SCOPE_EMPTY_HINTS[self.scope])
